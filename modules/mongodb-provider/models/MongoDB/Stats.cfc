...
Stats for MongoDB Provider
*/
component name="MongoDBStats" implements="coldbox.system.cache.util.ICacheStats" accessors="true"{
	...
	*/
	function init( cacheProvider ) output=false{
		...
		variables.instance.cacheStats = arguments.cacheProvider.getMongoDBClient().getStats().values().toArray();
		return this;
	}
	...
	void function clearStatistics() output=false{
		// not yet implemented by MongoDB
	}
